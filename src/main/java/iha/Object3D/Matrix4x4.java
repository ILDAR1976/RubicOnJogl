...
public class Matrix4x4 {
	...
	public static Point3D multF( Matrix4x4 a, Point3D b ) {
		// We treat the point as if
		// its (homogeneous) 4th component were one.
		return new Point3D(
			a.m[ 0]*b.x() + a.m[ 4]*b.y() + a.m[ 8]*b.z() ,
			a.m[ 1]*b.x() + a.m[ 5]*b.y() + a.m[ 9]*b.z() ,
			a.m[ 2]*b.x() + a.m[ 6]*b.y() + a.m[10]*b.z() 
		);
	}
	
	public static Point3D Normalaze(Point3D P){
		float invLength = 1/((float)Math.sqrt(P.x()*P.x()+P.y()*P.y()+P.z()*P.z()));
		return new Point3D(P.x() * invLength, P.y() * invLength, P.z() * invLength);
	}
}
